...
def console_menu():
    ...
    function_item1 = FunctionItem(
        "Démarrer un tournois", tournaments_informations, should_exit=True
    )
...
def tournament_console():
    ...
    tournaments_menu.append_item(
        FunctionItem(
            "Supprimer un tournois", remove_tournament, should_exit=True
        )
    )
        ...
        if self.turn == 1:
            while True:
                ...
                if name != "exit":
                    ...
                    continue
            self.players = sorted(
                self.players, key=attrgetter("ranking"), reverse=True
            )
            first_part = []
            second_part = []
            ...
            for i in range(len(first_part)):
                ...
                print(
                    "{:^11s} {:<11s}     s'oppose à {:^11s} {:<11s}".format(
                        first_part[i].name,
                        first_part[i].surname,
                        second_part[i].name,
                        second_part[i].surname,
                    )
                )
            ...
            if name != "exit":
                ...
                exit()
            self.players = sorted(
                self.players, key=attrgetter("score", "ranking"), reverse=True
            )
                ...
                while actual < len(self.players) and actual + 1 < len(
                    ...
                    while matched is False and (next < len(self.players)):
                        if self.conditions_duo(
                            self.players[actual], self.players[next], roundx
                        ):
                            matched = True
                            match = Match(
                                self.players[actual], self.players[next]
                            )
...
def add_player():
    ...
    new = Player(
        name=name, surname=surname, born=born, gender=gender, ranking=ranking
    )
    players.append(new)
    save_players()
            ...
            for result in round.results:
                ...
                for player in to_continue.players:
                    if player.name == name:
                        player.score += score
        for round in to_continue.rondes_instances:
            if len(round.results) != len(to_continue.players) or (
                len(round.results) == 0 and round.round_name is not None
            ):
                ...
                len(to_continue.rondes_instances) != to_continue.round
